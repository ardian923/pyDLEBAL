...
n 	= rs1.cell_value(117,2); n = int(n) ## number of nodes from 0 to 16
...
dt 	= rs1.cell_value(138,2); dt = float(dt) ## timestep
...
def solverWatEvapFlow(n, dt, flux, evap, psurface): 
    ...
    while (se > im): ## loop until mass balance error less than tolerance 
        ...
        for i in xrange(1, m+1): ## i = 1 to 15
            ...
            if (i == 1):
                D[i] = -(1-eps)*((qli[i-1]-qli[i])+(qvi[i-1]+qvi[i])) - cpu[i]*p[i] + (wu[i]-wiu[i])*(v[i]/dt) - (ResLCoefn[i]-ResLCoefn[i-1]) - \
